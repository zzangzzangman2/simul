...
- 주인공은 고정 인물 이미지를 생성하지 않는 무초상화 플레이어다. 김학준·한서윤·
  국정원/정부 관계자 등 나머지 NPC는 각 승인 규칙을 따른다. 윤하린 은행원과 서하늘
  공인중개사는 기존 얼굴·헤어·체형·의상·장신구·직업 소품을 반드시 유지하고 새 v3
  화풍의 선화·채색·명암·재질감으로만 다시 렌더링한다. 화풍이 맞지 않는
...
3. **여자 동기 8명일 때만** 교체할 현행 런타임 이미지를 **Image 3: 포즈·표정·체형·구도 참조**로 직접 연다. 사용자가 새 사복 참조를 명시한 교체 작업에서는 그 이미지를 다음 슬롯의 **승인 사복 참조**로 추가하고 기존 복장은 보존 대상에서 제외한다.
4. 그 밖의 인물은 구형 이미지를 정체성 참조로 넣지 않는다. 역할·나이·성격·장면 기능을
   텍스트로 설계하고, 꼭 필요한 소품이나 포즈만 제한된 참조로 추가한다.
5. 별도 전신 앵커, 사용자 승인 사복, 소품 또는 신발 참고가 꼭 필요할 때만 다음 Image 슬롯에 추가한다.
6. 프롬프트에 `Image 1에서는 렌더링 문법만 가져오고 한수아의 얼굴·머리·체형·복장·포즈는 복제하지 않는다`를 적는다.
7. 여자 동기 8명은 프롬프트에 `Image 2와 Image 3의 인물은 같은 사람이다. 얼굴·헤어·체형·표정·포즈·카메라·프레이밍은 유지한다`를 적는다. 사복 교체가 아닌 경우에만 기존 복장도 유지하고, 사복 교체 작업이면 `기존 복장 대신 승인 사복 참조의 디자인·색·재질·신발을 적용한다`를 명시한다.
...
- 데시멀 여자 동기 8명은 어떠한 경우에도 같거나 비슷한 얼굴을 사용할 수 없다. 머리와 개별 사복을 가린 얼굴만으로도 전원을 즉시 구분해야 한다.
...
4. 윤하린 은행원·서하늘 공인중개사는 기존 외형과 의상을 고정한 대표 포즈 한 장을
   v3 렌더링으로 먼저 승인한 뒤 같은 정체성으로 전체 세트를 확장한다.
5. 기존 런타임 파일은 대체 세트 전체가 준비되기 전까지 덮어쓰거나 연결 해제하지 않는다.
5. 적용 자산은 `ART_STYLE_AUDIT.md`에 기록하고 런타임에 연결한다.

## 7. 여자 8명 개별 사복·신발 규칙과 캐릭터별 체형 변주

- 여자 8명에게 공통 교복·공통 센터 현장복·공통 신발을 지정하지 않는다. 각 인물의
  최신 승인 정체성 앵커와 인물 문서에 적힌 사복을 그대로 사용한다.
- 2026-08-05 기준 김서아는 분홍 퍼프소매 블라우스·검정 체크 치마·흰 프릴 양말·검정
  메리제인, 오지우는 검정 오버핏 홈웨어 세트와 검정 슬라이드, 이지안은 흰색·남색
  운동복과 흰/남색 운동화가 승인 사복이다. 나머지 5명은 사용자가 새 참조를
  승인할 때까지 현행 의상을 임시 유지한다.
- 한 인물의 사복이나 신발을 다른 인물에게 복제하지 않는다. 새 참조가 없는 인물의
  의상을 임의로 재설계하지 않는다.
- 캐릭터별 개성은 승인 얼굴, 헤어, 키, 체중 인상, 어깨·골반 폭, 몸통 길이,
  팔·다리 길이, 머리 대 신체 비율, 골격과 개별 사복 실루엣으로 유지한다.
- 모든 체형 차이는 실제 중학교 1학년 연령에 맞는 건강하고 비선정적인 범위로 유지한다.
- 같은 캐릭터의 표정·동작 세트에서는 얼굴, 헤어, 키, 체형, 팔다리 비율, 승인 사복,
  신발과 소품을 고정한다. 장면마다 바꾸는 것은 표정, 손동작, 자세와 체중 이동뿐이다.
