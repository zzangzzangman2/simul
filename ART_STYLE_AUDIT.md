...
| 카지노 LIVE 중계 배경 | `flutter_app/assets/images/casino/` 로비·바카라·유럽식 룰렛·테이블 게임 4종 | **신규 생성·런타임 적용** — ImageGen 세로 소프트 페인터리 환경. 플레이어의 현장 방문이 아니라 요원 PC의 실시간 카메라 중계로 사용하며 인물·실제 상표·읽을 수 있는 간판 없음 |
...
배경에 각각 합성해 전수검수했다. 국정원·정부 인물 6종은 원본 해상도에서도 실루엣이
깨끗해 재생성 없이 유지했다. 별도로 교체한 한서윤 3포즈까지 포함해 이번 최종 검사
대상 81종은 체크무늬·흰 매트·검은 덩어리·밝은 외곽선이 없는 상태만 런타임에 반영한다.
...
완료했다. 김서아는 2026-08-05 분홍 쌍만두 정체성으로 9종 전체를 다시 교체했고,
