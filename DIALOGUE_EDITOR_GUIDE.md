...
`contentVersion: 4`, `appearanceVersion: 19`이며 정규 편집본은 294장면이다.
...
- `장면 연출 스튜디오`에서는 한 장면에 인물 레이어를 최대 6개까지 놓는다.
  레이어마다 화자·포즈·X/Y·크기·투명도·좌우 반전·앞뒤 순서·등장·퇴장·유휴
  동작을 따로 지정하며, 무대에서 선택한 인물을 바로 드래그하거나 휠로 확대한다.
- 카메라 탭은 팬 X/Y, 줌, 흔들림, 조명, 장면 전환과 전환 시간을 편집한다.
  환경 효과는 비·눈·먼지·조명 깜빡임을 지원하고 미리보기와 Flutter 런타임이
  같은 필드를 읽는다.
- 대사 연출 탭은 일반 대사·지문·속마음·시스템 창, 초당 글자 수와 자동 진행
  시간을 편집한다. 첫 탭은 타이핑을 즉시 완성하고 다음 탭부터 장면을 진행한다.
- 분기 탭은 기본 다음 장면과 최대 6개 선택지를 설정한다. 조건과 효과는
  `flag.route = true`, `호감도.한수아 += 2`처럼 한 줄씩 작성한다. 장면 검사는
  존재하지 않는 이동 대상과 시작점에서 도달할 수 없는 장면을 찾아 준다.
- 오디오·메모 탭은 BGM·효과음·음성 자산 경로, 음량, 태그와 제작 메모를
  정본에 보존한다. 저장 시 캐릭터 레이어와 오디오 경로까지 실제 에셋 존재 여부를
  검사한다. Flutter에서는 BGM을 장면 사이에 이어서 반복하고, 효과음과 음성은
  장면 진입 때 한 번 재생한다. 브라우저 자동재생 정책을 지키기 위해 이름 확정이나
  장면 진행 같은 사용자 입력 뒤 재생을 시작한다.
- 장면 목록 왼쪽 체크 버튼으로 여러 장면을 고른 뒤 태그·전환을 일괄 적용하고,
  복제·삭제·찾아바꾸기를 실행한다. 장면 템플릿과 전체 편집본 버전은 브라우저에
  각각 최대 20개까지 보관하며 복원 전에 확인한다.
...
1. 장면 수·필드 형식·중복 ID·6,000자 제한·분기 대상·실제 에셋 경로를 검증한다.
...
- Flutter는 편집본의 `order`를 기준으로 장면을 재생한다. 현재 정규본은
  294장면이며 마지막 장면을 동적으로 종료점으로 사용한다.
...
- 추가·삭제·재정렬된 장면과 각 장면의 배경·화자·포즈·지문·대사를 모두
  ...
  값은 거부한다.
- 인물 레이어가 있으면 단일 `character`보다 우선한다. 구형 편집본은 기존 단일
  캐릭터 필드를 자동으로 첫 레이어처럼 재생하므로 별도 마이그레이션이 필요 없다.
- 선택지는 현재 세션의 조건 변수를 검사하고 선택 효과를 적용한 뒤 지정 장면으로
  이동한다. 조건이 맞지 않는 장면은 다음 유효 장면까지 건너뛴다.
- 카메라·조명·전환·환경 효과·타이핑 속도·자동 진행과 오디오 큐는 게임
  런타임에서도 적용한다.
- 진행률, 다음 대사, 빠른 넘김과 완료 카드는 편집본의 실제 장면 수를 따른다.
- 장면 ID `decimal-final-ten-roster`는 최종 열 명 카드의 특수 연출 식별자다.
- `decimal-001`부터 `decimal-294`는 국정원 데시멀 재가동, 봉인된 실패 기록, 세 선발 시험, 강남 도착,
...
- 장면 연출 패널: `app/editor/director-studio.tsx`
- 일괄·버전·검사 패널: `app/editor/scene-toolbox.tsx`
- 편집 스키마: `app/editor/dialogue-types.ts`
