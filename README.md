...
프롤로그에서 플레이어 이름을 확정한 뒤 `1999년 국가 실험 → 데시멀 입소 → 첫날 밤 → DAY 1 첫 주문` 네 구간 중 시작 위치를 고릅니다. 각 구간 도착은 자동 저장되며, 저장 후 나가기와 요약 후 다음 구간 건너뛰기를 지원합니다. 2000년 1월 3일 각자의 CRT PC와 모의계좌로 딱 하루 연습한 뒤, 1월 4일부터 실제 50,000원 계좌로 주식, 관계, 메신저, 생활, 부동산과 사업을 하나의 날짜·월드시드 안에서 운영합니다.
