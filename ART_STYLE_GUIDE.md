...
- 체형도 공통 마네킹을 복제하지 않는다. 인물별로 `키`, `어깨 폭과 경사`, `몸통 길이`, `허리·골반의 폭`, `팔다리 길이`, `허벅지·종아리의 굵기와 근육감`, `평소 체중 중심`을 별도 정체성 명세로 고정한다. 얼굴과 교복을 가린 검은 실루엣만 비교해도 서로 구분돼야 하며, 같은 몸에 얼굴만 바꾼 결과는 자동 폐기한다. 모든 차이는 실제 중학교 1학년의 건강하고 비선정적인 범위 안에서 표현한다.
...
- 제6기 여학생의 현행 교복은 **배꼽과 복부를 완전히 가리는 정상 길이의 불투명 흰 반소매 단추 셔츠**를 쓰는 하복만 허용한다. 긴소매·크롭 셔츠·배 노출은 불합격이다.
- 기본 발 착장은 **흰 발목양말과 로고 없는 흰색 일반 로우톱 끈 스니커즈**다. 현실적인 끈과 고무 밑창을 사용하며 무양말·검정 로퍼·구두·부츠·클로그로 자동 보정한 결과는 불합격이다.
...
- 미래양성원 여자 SEED 교육생 교복의 실사 직물·재봉·체크 문양 기준:
  ...
  두 사진은 빨강 체크 넥타이·이중 랩 허리판·주름치마의 의상 전용 참조다. 사진 속 사람의 얼굴·피부·체형·포즈는 정체성 입력으로 사용하지 않고, 사진 속 긴소매나 크롭 상의도 현행 하복에 복사하지 않는다.
- 미래양성원 여자 SEED 교육생의 기본 발 착장은 흰 발목양말과 로고 없는 흰색 일반 로우톱 끈 스니커즈다. `future_development_female_pink_clogs_barefoot_reference_v1.png`와 무양말 규칙은 2026-08-01 중간안 보관본이며 현행 기준으로 사용하지 않는다.
- 최이서의 얼굴 절대 기준: `art_references/choi_iseo_face_anchor_v2.png`
- 최이서의 전신·현행 하복 절대 기준: `art_references/choi_iseo_summer_uniform_fullbody_master_v1.png`
- 최이서의 대사용 표정·포즈 보조 참조:
  - `art_references/original_girl_01_smile_pose_reference_v1.png`: 무섭지 않은 환한 웃음의 눈·볼·입 움직임과 한 손을 자연스럽게 든 밝은 포즈
  - `art_references/original_girl_01_neutral_hands_pose_reference_v1.png`: 힘을 뺀 눈썹·입매와 몸 앞에서 두 손을 느슨하게 모은 기본 대화 포즈
  - `art_references/original_girl_01_soft_neutral_uniform_reference_v1.png`: 부드러운 기본 표정과 팔을 내린 전신 인상만 참고한다. 이 사진의 상의 길이는 현행 하복에 복사하지 않는다.
  - `art_references/original_girl_01_required_bright_smile_pose_v2.png`: **반드시 포함할 환한 웃음 포즈**. 자연스럽게 보이는 치아, 접히는 눈, 올라간 볼, 살짝 기울인 머리, 머리 옆으로 가볍게 든 오른손과 힘을 뺀 몸의 체중 이동을 그대로 사용한다.
  네 사진은 표정·포즈 역할만 담당하며 사진 속 사람의 얼굴 정체성·긴소매·크롭 상의·검정 로퍼는 복사하지 않는다.
- 최이서의 다양한 대사용 포즈는 허수아비처럼 양팔을 반복해서 내리지 않는다. 대사 기능에 따라 한 손 인사, 두 손 모음, 질문하는 손, 놀란 양손, 팔짱, 턱 가까이 생각하는 손, 결의한 작은 주먹, 시선을 피한 걱정 자세처럼 손·팔·어깨·머리 기울기·체중 중심을 함께 바꾼다. 단, 얼굴은 `choi_iseo_face_anchor_v2.png`, 하복·양말·신발은 `choi_iseo_summer_uniform_fullbody_master_v1.png`와 동일하게 고정한다.
