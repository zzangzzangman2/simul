...
test("opens the Flutter Project Decimal prologue from the default route", async () => {
  ...
  assert.doesNotMatch(dialogueBundleRaw, /protagonist_seed01\//);
...
test("uses the deterministic local news combinator without a remote API", async () => {
  ...
  assert.match(flutterPubspec, /^\s+http:\s*\^1\.6\.0/m);
  assert.doesNotMatch(`${combinator}${marketNews}${campaignScenes}`, /\/api\/news/);
  assert.doesNotMatch(`${combinator}${marketNews}${campaignScenes}`, /package:http/);
  assert.match(main, /PhoneAiService/);
...
test("keeps Gemini keys out of source and provides secure device setup plus local fallback", async () => {
  ...
  assert.doesNotMatch(route, /gemini-3\.6-flash/);
...
test("ships an intuitive dialogue editor and builds saved dialogue into the game", async () => {
  ...
  const hakjunAssets = (
    await readdir(
      new URL(
        "../flutter_app/assets/images/production_soft_painted/kim_hakjun/",
        import.meta.url,
      ),
    )
  )
  ...
  assert.match(editor, /이 캐릭터만 적용/);
  assert.match(editor, /같은 캐릭터 전체 적용 ·/);
  assert.match(editor, /scene\.characters\?\.some\(\(layer\) => layer\.speaker === selected\.speaker\)/);
  assert.match(editor, /onUpdateLayer=\{updateCharacterLayer\}/);
  ...
  assert.match(editor, /const currentCharacters = scene\.characters\?\.length/);
  assert.match(editor, /id: "primary"/);
  assert.match(editor, /characterCoordinateBadge/);
  assert.match(editor, /좌표 복사/);
  assert.match(editor, /beginCharacterResize/);
  assert.match(editor, /크기 드래그 조절/);
  assert.match(editorCss, /\.characterDragHandle\s*\{/);
  assert.match(editorCss, /\.characterCoordinateBadge\s*\{/);
  assert.match(editorCss, /\.characterScaleHandle\s*\{/);
  assert.match(editorCss, /\.characterCoordinateBadge\s*\{[\s\S]*?top: 76px/);
  assert.match(editorCss, /\.characterScaleHandle\s*\{[\s\S]*?top: 42px/);
  ...
  assert.match(editorCss, /\.characterManipulator\s*\{[\s\S]*?bottom: 0[\s\S]*?height: 90%/);
  assert.match(editor, /const STORY_CHARACTER_SCENE_SCALE = 1\.45/);
  assert.match(editor, /const STORY_CHARACTER_SCENE_OFFSET_PERCENT = 8/);
  assert.match(editor, /STORY_CHARACTER_SCENE_SCALE \* layer\.scale/);
  assert.match(editor, /rgba\(2,8,18,\.32\)/);
  ...
  assert.match(editor, /const SCENES_PER_PAGE = 24/);
  assert.match(editor, /filteredScenes\.slice\(start, start \+ SCENES_PER_PAGE\)/);
  assert.match(editor, /장면 목록 페이지/);
  assert.match(editor, /번만 표시/);
  assert.match(editor, /const sceneFingerprintCache = new WeakMap/);
  assert.match(editor, /const sceneIds = new Set<string>/);
  assert.match(editor, /useDeferredValue/);
  assert.match(editor, /scheduleCharacterUpdate/);
  assert.match(editor, /window\.requestAnimationFrame/);
  assert.match(editor, /previewStageInteracting/);
  assert.match(editorCss, /\.scenePagination\s*\{/);
  assert.match(editorCss, /content-visibility: auto/);
  assert.match(editorCss, /\.previewStageInteracting \.ambientLayer/);
  assert.match(editorCss, /\.characterApplyScope\s*\{/);
  assert.match(editorCss, /\.previewDialogue\s*\{[\s\S]*?min-height: 154px/);
  assert.match(editorCss, /\.previewDialogue\s*\{[\s\S]*?backdrop-filter: blur\(3\.2px\)/);
  assert.match(editorCss, /\.previewDialogue\s*\{[\s\S]*?rgba\(13, 28, 45, \.34\)/);
  assert.match(editorCss, /\.previewDialogue > b\s*\{[\s\S]*?background: transparent/);
  ...
  assert.match(directorStudio, /onUpdateLayer/);
  ...
  assert.ok(playerScenes.length > 0);
  assert.ok(playerScenes.every((scene) => scene.character === ""));
  const hakjunScenes = canonical.scenes.filter((scene) => scene.speaker === "김학준");
  assert.ok(hakjunScenes.length > 0);
  const hakjunSceneAssets = new Set(
    hakjunScenes.map((scene) => scene.character.split("/").at(-1)),
  );
  assert.deepEqual([...hakjunSceneAssets].sort(), expectedHakjunAssets);
  ...
  assert.doesNotMatch(data, /character_hakjun_orientation_v2\.png/);
  ...
  assert.equal((protagonistCatalog.match(/\.png/g) ?? []).length, 0);
  assert.match(protagonistCatalog, /pose\("00", "인물 없음", ""\)/);
