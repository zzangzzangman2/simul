...
test("opens the Flutter family-story prologue from the default route", async () => {
  ...
  assert.match(onboarding, /가격이 제일 많이 오른 회사가 제일 좋은 회사예요/);
  assert.match(onboarding, /시장가랑 지정가는 이름이 어려워요/);
  assert.match(onboarding, /투자노트 표지에 붙일 이름도 하나 정해 볼까요/);
  assert.match(onboarding, /투자노트 이름을 적고 주문 연습 시작/);
