...
- 모바일 1열 종목 목록, 종목 상세, 매수·매도와 포트폴리오
- 국내 22개 실제 종가 기본 팩과 별도 해외 탭 6개, 날짜별 역사 소식 400여 건
...
Flutter 테스트 52개가 게임 엔진, 저장 마이그레이션, 스토리, 미니게임, 시장 소식, 경영진 데이터와 화면 동작을 검증합니다. 레이아웃 회귀 테스트는 `360×800`과 `390×844` 세로 화면에서 배경·인물·카드·종목 상세가 화면 밖으로 넘치지 않는지 확인합니다.
...
Flutter Web은 같은 호스트의 `/api/news`를 자동으로 사용합니다. Android 빌드는 배포된 서버 주소를 `--dart-define=NEWS_API_BASE_URL=https://서버주소`로 전달합니다. AI 서버가 없거나 16초 안에 응답하지 않으면 기존 역사·시장 신문으로 자동 폴백하므로 하루 진행은 막히지 않습니다.