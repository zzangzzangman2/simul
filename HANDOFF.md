...
- Flutter 상태 스키마: 버전 13
- 기존 React 앱은 회귀 비교용으로 보존하며 React 웹 스키마 v3 진행은 Flutter v13으로 가져옵니다.
- 로컬 환경마다 Flutter 안정 버전을 설치하고 `flutter --version`으로 3.44.7 이상인지 확인
- 웹 기본 주소 `/`는 `/play/index.html`의 Flutter 타이틀로 이동하며, `처음하기`는 가족 프롤로그를 열고 `이어하기`는 최대 5개 로컬 저장을 보여 줍니다.
...
- 1999-12-31 23:57 작은방에서 시작해 거실·식탁·다시 작은방으로 이어지는 24단계 가족 대화 프롤로그
...
- 2000-01-02 일요일 08:00에 초기자본 0원, 창립 인원 1명으로 시작하며 첫 하루 진행 뒤 2000-01-03 월요일 08:00에 시장을 이용. 기존 저장 달력은 그대로 보존
...
- 닉네임·연구소 이름 입력 시 배경과 상단 표시를 고정하고 이름 패널만 모바일 키보드 바로 위로 한 번 이동
- 가족 신뢰, 가족별 관계, 보호자 계좌 권한과 회사문화 태그 저장
- 쉬운 첫 미션과 제품·돈 버는 법·운영자·가격 관점 선택, 선택지 뒤 펼쳐보는 가족 의견
...
- 종목 상세의 실제 일별 종가와 분리된 게임용 1분봉, 장 상태·전일 대비·기업 요약·사기/팔기·부모 주문 승인 시트. 시장 화면에서만 현실 1초당 게임 1분 진행하며 이체·주문 시트가 열리면 정지
...
- 부엌 유선전화의 `일거리`에서 설거지 순서 암기·문방구 품목 분류·가족 벼룩장터 거스름돈 3종 미니게임
- 2000년 상반기 시간급 1,600원 기준, 점수별 300~1,600원 기본 수당과 성향·스킬 보너스, 날짜별 3회 제한과 현금 장부 저장
- 0원에서 일거리로 누적 1만원을 직접 벌어 첫 주문 권한을 열고, 이후 계속 일할지 조사·투자에 집중할지는 플레이어가 결정
...
- Flutter 상태 v13에 `StoryState`, `OrganizationState`, 포지션과 AUM 원장을 포함합니다.
- 기존 논리 저장 키를 1번 슬롯으로 유지하고 파생 키를 사용하는 최대 5슬롯입니다. 타이틀 이어하기에서 요약·불러오기·삭제가 가능하며 슬롯별 백업과 손상 원문을 관리합니다.
- 날짜가 하루 넘어갈 때마다 활성 슬롯에 자동저장하고, 허브 상단 메뉴에서 현재 슬롯 수동저장과 타이틀 복귀를 제공합니다.
- 이전 Flutter 저장은 회사명·날짜·현금·조직을 보존하고, React 웹 v3 저장은 회사명·날짜·현금·포지션과 숫자형 팀을 Flutter v13 모델로 가져옵니다. 아직 대응 모델이 없는 React 전용 필드는 가져온 것으로 설명하지 않습니다.
- 이전 Flutter 저장의 현금·날짜·시각·안건·포지션·원장은 그대로 보존하며 자본을 임의로 지급하지 않습니다. 레거시 해외 포지션은 기록된 원가를 원화 현금으로 한 번 복구합니다.
- 모든 게임 상태 변경은 플랫폼 저장 성공 뒤에만 화면에 반영합니다. 저장 API가 `false`를 반환하거나 예외가 나면 기존 화면 상태를 유지하고 실패 안내를 표시합니다.
- 웹 저장 브리지는 Flutter 부트스트랩 전에 한 번만 실행되며 기존 Flutter 저장이 없을 때만 React v3 문자열을 1번 슬롯 키로 복사하고 완료 마커를 남깁니다. 이미 있는 Flutter 진행은 덮어쓰지 않고, 이후 1번 슬롯을 삭제해도 예전 React 저장을 다시 주입하지 않습니다.
- 자동 검증은 엔진·거래·이전 Flutter 저장→v13 보존 마이그레이션·주식시장의 시스템 Back 저장·경영진·시장소식·위젯·모바일 레이아웃을 포함하며 테스트 개수와 통과 여부는 최종 통합 검증 결과를 기준으로 확인합니다.
...
- 기존 React 앱은 v13 가져오기와 회귀 검증의 기준이므로 별도 결정 전까지 삭제하지 않습니다.
...
- 저장 모델·카탈로그: `flutter_app/lib/game/personal_finance_state.dart`
- 구매·매각·월 결산·확률 결과: `flutter_app/lib/game/game_engine.dart`
- 모바일 화면: `flutter_app/lib/asset_spending_screen.dart`
- 진입: 작은방 서류함 → 포트폴리오 상단 `자산·소비 계획`
- 영속 상태: Flutter v13의 `personalFinance`

부동산 수치와 매각가는 실제 데이터가 아니라 `BALANCE_NOTES.md`에 공개한 게임용 수치다. 주식 AUM과 합산하지 않는다. 성인 확률 오락은 2010년·20세 이상·월 1회·최소 1만원·현금 1%/10만원 상한을 모두 엔진에서 다시 검사한다. UI 잠금만 믿지 않는다.
...
모델은 `gemini-3.6-flash`이며 `Downloads/ai`의 3.6 Flash 설정과 같이 MEDIUM 추론을 사용하고 3.6에서 폐기된 샘플링 파라미터는 보내지 않는다. API 키를 앱이나 저장소에 넣지 않는다. 로컬은 Vertex AI 프로젝트·위치 환경변수와 gcloud ADC를 사용한다. 일반 로컬 실행은 Node 런타임이 필요한 `npm run dev`를 사용하고, `npm run dev:sites`에서는 AI가 폴백될 수 있다. `stockImpactScore`는 -30~+50의 서사 입력값일 뿐 현재 실제 종가를 직접 바꾸지 않는다.
