...
`market_corpus_daily_samples.dart`의 6,545거래일 표본은 두 경로로 일봉에 들어간다.
시장 전체 공통항은 지수 수익률을 배율 없이(±6% 클램프) 쓰고, 종목별 항은 대형주·성장주
채널을 0.70배로 더한다. 개별 종목이 지수보다 크게 움직이는 실제 성질을 종목별 항이 맡는다.

날짜 대응은 하지 않는다. 표본을 42거래일 블록으로 잘라 시드로 재배치하므로 블록 안의
변동성 군집만 살아남고, 게임의 2008년이 실제 2008년을 재현하지는 않는다. "새 게임은
미래가 달라진다"를 지키기 위한 의도적 선택이다.

배율을 바꾸면 `market_100_day_excitement_test.dart`의 분포 계약이 함께 움직이므로
`BALANCE_NOTES.md`와 같은 변경에서 갱신한다. 그 감사는 시드 하나가 아니라 여러 시드를
합산해 검증한다. 예전에는 고정 시드 하나만 봤는데, 그 시드가 전형적인 시드보다 변동이
몇 배 큰 이상치여서 실제 플레이어 시장이 계약 미달인데도 통과했다.
