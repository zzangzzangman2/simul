...
`app/data/market-history.json`과 `flutter_app/assets/market/market-history.json`은 같은 스키마 4 원장을 사용합니다. `prices`는 거래일의 원시 일별 종가이고, `adjustedIndex`는 분할·배당을 반영한 수정주가를 기준일=100으로 만든 보조 수익률 지수입니다. `corporateActions`에는 Yahoo Finance chart event에서 받은 분할·주식배당 20건과 현금배당 279건을 종목 ID, 효력일, 비율·금액, 통화, 출처와 함께 저장합니다.
...
현재 국내 22개는 전 종목 완전판이 아니라 Yahoo Finance chart endpoint에서 2000~2010 원시 종가를 검증할 수 있었던 개발용 기본 팩입니다. 2000-01-03은 Y2K 금융휴무일이었고 새해 증시는 1월 4일에 개장했습니다. 삼성전자의 첫 제공 거래일도 2000-01-04이며 종가는 6,110원입니다. 종목별 첫 관측 전에는 미래 가격을 미리 보여주지 않습니다. 개장일 근거는 [1999년 당시 보도](https://www.mk.co.kr/news/all/2257263)와 `flutter_app/test/market_data_test.dart`의 미래 시세 차단 회귀 테스트를 함께 유지합니다.

Flutter의 1분 장중 틱은 실제 체결 데이터가 아닙니다. 주식시장 화면이 열려 있을 때 현실 1초마다 게임 시각을 1분씩 진행합니다. 08:00~08:59는 이전 종가로 고정하고 주문과 분봉 생성을 막으며, 09:00부터 생성한 경로를 15:30의 450번째 틱에서 실제 당일 종가에 고정합니다. 15:30 이후에는 별도 확장장을 만들지 않고 20:00까지 같은 종가를 유지합니다. 화면을 닫으면 주기 타이머를 제거하고, 주말·휴장일에는 직전 종가를 유지합니다.

Flutter v13의 장중 주문과 AUM은 화면에 보이는 현재 생성 틱으로 계산합니다. 09:00 전과 15:30 이후에는 주문할 수 없고, 15:30 틱은 확정된 실제 일별 종가와 같습니다. 생성 장중 틱과 Gemini 기사 영향점수는 실제 종가 원장을 덮어쓰지 않으며 화면에서도 게임용 연출로 구분합니다. 주문 저장 직전에는 공용 권위 시세 서비스가 원장에서 종목, 날짜, 분, 생성 가격을 다시 계산합니다. 날짜 진행 시 현금배당은 효력일 직전 보유 수량을 기준으로 현금 원장에 반영하고, 같은 날 분할은 그 뒤 수량에 적용하며 총원가는 유지합니다. 처리한 이벤트 ID를 저장해 새로고침해도 중복 지급하지 않습니다. USD·JPY를 원화처럼 합산하지 않도록 해외 주문과 원화 AUM 환산은 실제 환율 원장을 연결하기 전까지 잠급니다.
...
- 구현 주의: NXT는 2000년 역사 데이터가 아니며 현재 게임에는 NXT형 확장장을 사용하지 않습니다. 15:30 이후에는 주문·분봉 없이 실제 종가를 20:00까지 유지합니다.
