...
Flutter의 1분 장중 틱은 실제 체결 데이터가 아닙니다. 주식시장 화면이 열려 있을 때 현실 1초마다 게임 시각과 생성 가격을 1분씩 진행합니다. 08:00 이전 종가에서 출발해 15:30의 450번째 틱을 실제 당일 종가에 고정하고, NXT형 확장장은 종가 주변에서 변동한 뒤 20:00의 720번째 틱에 다시 실제 종가로 닫습니다. 화면을 닫으면 주기 타이머를 제거하고, 주말·휴장일에는 틱을 만들지 않고 직전 종가를 유지합니다.

Flutter v13의 장중 주문과 AUM은 화면에 보이는 현재 생성 틱으로 계산합니다. 15:30 틱은 확정된 실제 일별 종가와 같고, 15:40 이후 확장장은 다시 게임용 생성 가격으로 표시하다가 20:00에 실제 종가로 닫습니다. 주문창에서 장 마감값을 미리 보여주거나 확장장 가격을 실제 종가로 부르지 않습니다. 생성 장중 틱과 Gemini 기사 영향점수는 실제 종가 원장을 덮어쓰지 않으며, 화면에서도 게임용 연출로 구분합니다. 주문 저장 직전에는 공용 권위 시세 서비스가 원장에서 종목, 날짜, 분, 생성 가격을 다시 계산합니다. 날짜 진행 시 현금배당은 효력일 직전 보유 수량을 기준으로 현금 원장에 반영하고, 같은 날 분할은 그 뒤 수량에 적용하며 총원가는 유지합니다. 처리한 이벤트 ID를 저장해 새로고침해도 중복 지급하지 않습니다. USD·JPY를 원화처럼 합산하지 않도록 해외 주문과 원화 AUM 환산은 실제 환율 원장을 연결하기 전까지 잠급니다.
