...
이 문서는 게임에 포함된 BGM·효과음의 출처와 라이선스를 보관하는 개발 문서다.
게임 화면, 타이틀, 설정, 엔딩에는 저작권·크레딧 문구를 노출하지 않는다.
...
| `horse_racing_prairie4.ogg` | Prairie4 (official loop) | <https://peritune.com/blog/2019/03/01/prairie4/> | 경마장·경주 |
| `casino_taisho.ogg` | TaishoRoman Theme2 | <https://peritune.com/blog/2020/11/19/taishoroman_theme2/> | 카지노 |

음원은 원본 OGG를 파일명만 바꿔 사용하며, 반복 재생과 런타임 볼륨 조절 외의
편집은 하지 않았다.
...
| Impact Sounds | <https://kenney.nl/assets/impact-sounds> | 경주 벨, 충돌, 타격 |
...
`ground.mp3`만 사용해 3.09초 OGG 루프로 변환했다. 묶음 안의 CC BY 파일은
게임에 포함하지 않았다.

## 제외한 음원

테일즈위버 원곡은 무료 청취·OST 공개와 게임 내 재배포 허가는 별개이고,
넥슨이 OST 저작권을 보유한다고 명시하므로 포함하지 않았다. 명시적인 재사용
허가를 확보하기 전에는 프로젝트에 복사하지 않는다.
